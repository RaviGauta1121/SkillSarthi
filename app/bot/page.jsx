  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (err.message.includes('API key')) {
        errorMessage = '🔑 API key issue. Please check your environment variables.';
        ...
        <div className="h-96 overflow-y-auto p-4 bg-gray-50">
          {chat.length === 0 ? (
            <div className="text-center text-gray-500 mt-20">
              ...
              <p className="text-sm mt-2">Ask questions, get creative ideas, or just chat.</p>
            </div>
          ) : (
            chat.map((msg, index) => (
              <div key={index} className={`mb-4 ${msg.type === 'user' ? 'text-right' : 'text-left'}`}>
                <div className={`inline-block max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-lg ${
                  msg.type === 'user' 
                ...
                <div className={`text-xs text-gray-600 mt-1 ${msg.type === 'user' ? 'text-right' : 'text-left'}`}>
            ...
            <div className="text-left mb-4">
              <div className="inline-block bg-white shadow-md px-4 py-2 rounded-lg rounded-bl-none">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-500 text-sm">Gemini is thinking...</span>
                </div>
              </div>
            </div>
          )}
